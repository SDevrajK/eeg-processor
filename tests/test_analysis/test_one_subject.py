...
def run_test_pipeline(config_path):
    processor = EEGPipeline(config_path)
    processor.run()
    return processor.processor.raw, processor.processor.epochs

if __name__ == "__main__":
    config_path = Path(__file__).parent.parent / "test_config/test_processing_params.yml"
    raw, epochs = run_test_pipeline(config_path)

    sub = 0
    cond = 0

    # Load data
    stan = epochs[epochs.metadata['event_type']=='Standards']
    dev = epochs[epochs.metadata['event_type']=='Deviants']
